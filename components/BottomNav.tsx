...
const BottomNav: React.FC<BottomNavProps> = ({ currentScreen, onScreenChange, onAddClick }) => {
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-white dark:bg-surface-dark border-t border-gray-100 dark:border-gray-800 px-6 py-3 flex justify-between items-center z-50 rounded-t-[2rem] shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
      ...
      <button 
        onClick={() => onScreenChange(AppScreen.SillySoundboard)}
        className={`flex flex-col items-center gap-1 p-2 transition-colors ${currentScreen === AppScreen.SillySoundboard ? 'text-primary' : 'text-gray-400'}`}
      >
        <span className={`material-symbols-outlined ${currentScreen === AppScreen.SillySoundboard ? 'filled' : ''}`}>graphic_eq</span>
        <span className="text-[10px] font-bold">Sounds</span>
      </button>

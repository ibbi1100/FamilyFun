      ...
      <header className="flex items-center justify-between p-6 pb-2 relative z-20">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            onClick={async () => {
              if (isSpinning) return;
              const prompt = `Generate ONE funny, made-up name for a monster that has a ${selectedHead.name} head and ${selectedSkin} skin. Just the name, nothing else.`;
              try {
                const newName = await getGroqCompletion(prompt, 20);
                if (newName) setName(newName.replace(/["\.]/g, ''));
              } catch (e) {
                console.error(e);
              }
            }}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-primary/20 text-primary-dark hover:bg-primary hover:text-black transition-colors"
          >
      ...
      <div className="bg-white dark:bg-surface-dark rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.1)] pb-8 pt-8 px-6 relative z-30">
        <div className="flex overflow-x-auto no-scrollbar gap-4 mb-6">
          {categories.map(cat => (
            <button
              ...
              disabled={isSpinning}
              className={`px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest whitespace-nowrap transition-all disabled:opacity-50 ${selectedCategory === cat
        ...
        <div className="flex overflow-x-auto no-scrollbar gap-6 py-4 min-h-[140px]">
          {selectedCategory === 'Heads' ? MONSTER_HEADS.map(head => (
            ...
            >
              <div className={`w-24 h-24 rounded-3xl flex items-center justify-center border-4 transition-all ${selectedHead.id === head.id ? 'bg-primary/10 border-primary shadow-xl' : 'bg-gray-50 dark:bg-white/5 border-transparent'
                }`}>

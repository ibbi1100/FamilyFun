...
import { generateCrazyChallenge, enhanceChallenge } from '../services/aiService';
...
const ChallengeCreator: React.FC<ChallengeCreatorProps> = ({ onCreate }) => {
  ...
  const handleEnhance = async () => {
    ...
    setIsEnhancing(true);
    try {
      const enhanced = await enhanceChallenge(title);
      ...
      <div className="space-y-6">
        <div className="relative flex gap-2">
          <input
            ...
            className="flex-1 bg-gray-100 dark:bg-white/5 border-none rounded-2xl px-6 py-4 text-lg font-black placeholder-gray-400 focus:ring-4 focus:ring-primary/20 transition-all shadow-inner"
            placeholder="Challenge Name (e.g. Do jumps)..."
            type="text"
          ...
          <button
            ...
            className="bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300 rounded-2xl px-4 flex items-center justify-center hover:bg-purple-200 transition-colors disabled:opacity-50"
            title="Enhance with AI"
          >
            <span className={`material-symbols-outlined ${isEnhancing ? 'animate-spin' : ''}`}>auto_fix_high</span>
          </button>
        </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {xpOptions.map(opt => (
              <button
                key={opt}
                onClick={() => setXp(opt)}
                className={`py-3 rounded-xl font-black text-sm transition-all border-2 ${xp === opt
